...
@application.route('/taskss', methods=['GET'])
def get_weathers():
    return jsonify({'weather': weather})

@application.route('/historical/', methods=['GET'])
def get_dates():
    task = [{"DATE": task['DATE']} for task in weather]
    return jsonify(task)

@application.route('/historical/<dateYYYYMMDD>', methods=['GET'])
def get_particulardates(dateYYYYMMDD):
    task = [task for task in weather if task['DATE'] == dateYYYYMMDD]
    if len(task) == 0:
        abort(404)
...
@application.route('/plot/<YYYYMMDD>', methods=['GET'])
def get_plot(YYYYMMDD):
...
@application.route('/historical/<YYYYMMDD>', methods=['DELETE'])
def delete_date(YYYYMMDD):
    task = [task for task in weather if task['DATE'] == YYYYMMDD]
    if len(task) == 0:
        abort(404)
    for i in task:
        weather.remove(i)
...
def create_data():
   ...
   existing = [existing for existing in weather if existing['DATE'] == task['DATE']]
   if len(existing) != 0:
       for i in existing:
       	   weather.remove(i)
   weather.append(task)
